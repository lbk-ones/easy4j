 ...
 */
package easy4j.infra.dbaccess.dll.op.impl.mp;
...
import easy4j.infra.dbaccess.dll.op.api.MetaInfoParse;
...
public class JavaClassMetaInfoParse implements MetaInfoParse {
    ...
    private DDLTableInfo getDdlTableInfo() throws SQLException {
        ...
        List<DatabaseColumnMetadata> columns = dialect.getColumnsNoCache(this.opContext.getConnectionCatalog(), this.opContext.getConnectionSchema(), ddlTableInfo.getTableName());
        this.opContext.setDbColumns(columns);
        List<PrimaryKeyMetadata> primaryKes = dialect.getPrimaryKes(this.opContext.getConnectionCatalog(), this.opContext.getConnectionSchema(), ddlTableInfo.getTableName());
        ...
        List<TableMetadata> tableInfos1 = dialect.getTableInfos(ddlTableInfo.getTableName());
    ...
    public void fieldMetaInfoExtraParse(DDLTableInfo ddlTableInfo, Object newInstance, Field field, DDLFieldInfo ddlFieldInfo, Map<String, DDLIndexInfo> columnVsIndexMap) {
        ...
        if (opConfig.isDb2AutoUpperCase() && Objects.equals(this.opContext.getDbType(), DbType.DB2.getDb())) {
            ddlFieldInfo.setName(ddlFieldInfo.getName().toUpperCase());
        }
        // patch index name
