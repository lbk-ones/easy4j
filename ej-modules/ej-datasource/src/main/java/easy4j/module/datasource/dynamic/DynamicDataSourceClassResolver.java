...
public class DynamicDataSourceClassResolver {
     ...
     */
    public DynamicDataSourceClassResolver(boolean allowedPublicOnly) {
    ...
    private <T> BasicAttribute<T> findDataSourceAttribute(AnnotatedElement ae, Class<? extends Annotation> annotation) {
        if (annotation.isAssignableFrom(DataSource.class)) {
            //AnnotatedElementUtils.findMergedAnnotation()会委托给findMergedAnnotationAttributes()
            DataSource ds = AnnotatedElementUtils.findMergedAnnotation(ae, DataSource.class);
            if (ds != null) {
