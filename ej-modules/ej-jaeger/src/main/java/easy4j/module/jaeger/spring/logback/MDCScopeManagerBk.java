...
public class MDCScopeManagerBk implements ScopeManager {
  ...
  private MDCScopeManagerBk(Builder builder) {
  ...
  /**
   * Builds an {@link MDCScopeManagerBk} with options.
   * Calling {@code new MDCScopeManager.Builder().build()}
   * Builds an {@link MDCScopeManagerBk} with configuration as follows:
  ...
  public static class Builder {
    ...
    public MDCScopeManagerBk build() {
      return new MDCScopeManagerBk(this);
    }
  ...
  private class MDCScope implements Scope {
    ...
    protected void putContext(JaegerSpanContext spanContext) {
      replace(mdcTraceIdKey, spanContext.toTraceId()+":"+spanContext.toSpanId());
