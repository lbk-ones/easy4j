...
public class BaseTccAction {
    ...
    public void tccDegrade(NullConsumerCallback consumer) {
        ...
        if ((!SeataUtils.isInGlobalTransaction() || null == context) && consumer != null) {
            boolean hasAnnotation = AnnotationUtils.getAnnotation(this.getClass(), LocalTCC.class) != null;
            if (!hasAnnotation) {
                return;
            }
            ...
            for (Method method : methods) {
                TwoPhaseBusinessAction annotation = AnnotationUtils.getAnnotation(method, TwoPhaseBusinessAction.class);
                if (annotation != null) {
