...
import io.github.lbkones.cloud.openfeign.PublicHeaders;
import lombok.Data;
...
/**
 * 从nacos中找服务调用地址，然后使用RestTemplate进行调用
 *
 * @author bokun.li
 * @since 2.1.4
 */
...
public class NamingServerInvoker extends StandAbstractEasy4jResolve implements AutoRegisterContext, Easy4jNacosInvokerApi {
    ...
    public static NamingServerInvoker createByEnv(RestTemplate restTemplate) {
        if (HOLDER == null) {
            synchronized (NamingServerInvoker.class) {
                if (HOLDER == null) {
                    NacosPropetiesParse build = NacosPropetiesParse.build(null, false);
                    String nacosDiscoveryUrl = build.getNacosDiscoveryUrl();
                    String nacosDiscoveryNameSpace = build.getNacosDiscoveryNameSpace();
                    String nacosDiscoveryUsername = build.getNacosDiscoveryUsername();
                    String nacosDiscoveryPassword = build.getNacosDiscoveryPassword();
                    HOLDER = new NamingServerInvoker(
                            nacosDiscoveryNameSpace,
                            nacosDiscoveryUrl,
                            nacosDiscoveryUsername,
                            nacosDiscoveryPassword,
                            restTemplate
                    );
                }
            }
    ...
    @Override
    public String get(NacosInvokeDto nacosInvokeDto) {
        ...
        if (StrUtil.isNotBlank(accessToken)) {
            return get(serverName, group1, path, paramMap, accessToken);
        } else {
            return get(serverName, group1, path);
        }
    }

    @Override
    public String post(NacosInvokeDto nacosInvokeDto) {
        String group1 = nacosInvokeDto.getGroup();
        Map<String, Object> paramMap = nacosInvokeDto.getParamMap();
        String accessToken = nacosInvokeDto.getAccessToken();
        ...
        if (nacosInvokeDto.isJson()) {
            return postJson(serverName, group1, path, body, accessToken);
        } else {
            return postForm(serverName, group1, path, paramMap, accessToken);
        }
    }

    @Override
    public String put(NacosInvokeDto nacosInvokeDto) {
        String group1 = nacosInvokeDto.getGroup();
        Object body = nacosInvokeDto.getBody();
        String path = nacosInvokeDto.getPath();
        String serverName = nacosInvokeDto.getServerName();
        if (nacosInvokeDto.isJson()) {
            return putJson(body, serverName, group1, path);
        }
        return null;
    }

    @Override
    public String delete(NacosInvokeDto nacosInvokeDto) {
        String group1 = nacosInvokeDto.getGroup();
        String accessToken = nacosInvokeDto.getAccessToken();
        String path = nacosInvokeDto.getPath();
        String serverName = nacosInvokeDto.getServerName();
    ...
    public void registerToContext(Easy4jContext easy4jContext) {
        // 不用代理 直接这样就行
        easy4jContext.register(createByEnv(SpringUtil.getBean(RestTemplate.class)));
    }
}